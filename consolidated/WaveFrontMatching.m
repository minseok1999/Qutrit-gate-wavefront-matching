...
for PhScrInd = 1:1:PhaseScreenNum % For all phase screens
    ...
    [Beam(:,:,PhScrInd+1),pixel1]=Scalar_Bluestein(Beam(:,:,PhScrInd+1),nx,ny,PixelSize,PropDist,x1start,x1end,y1start,y1end,nx,ny);
end
...
for PhScrInd=PhaseScreenNum:-1:2 % For all phase screens
    ...
    if PhScrInd == PhaseScreenNum % = For final output mode
       ...
       [BeamBack(:,:,PhaseScreenNum),pixel1]=Back_Bluestein(abs(ModesOut).*...
            exp(-(1i*angle(ModesOut))),nx,ny,PixelSize,PropDist,x1start,x1end,y1start,y1end,nx,ny);
   ...
   [BeamBack(:,:,PhScrInd-1),pixel1]=Back_Bluestein(Beam(:,:,PhScrInd-1),nx,ny,PixelSize,PropDist,x1start,x1end,y1start,y1end,nx,ny);
end
