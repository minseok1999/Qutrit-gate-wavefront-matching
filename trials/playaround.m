...
% SLM specifications:
    ...
    PhaseScreenNum = 7; % Number of Phase screens used in optimization
...
ModesOut = GenModesLG([1 0], w_out, Rad, Angle);
...
for PhScrInd=PhaseScreenNum:-1:2 % For all phase screens
    ...
    if PhScrInd == PhaseScreenNum % = For final output mode
        ...
        BeamBack1(:,:,PhaseScreenNum) = SplitStepProp(ModesOut,KZ,PropDist);
    end
...
% Check overlap and visibility, and plot current values
    ...
    Overlap_to_output = real(abs(sum(sum(conj(Beam1(:,:,PhaseScreenNum+1)).*ModesOut))).^2);    
    resultoverlap1(IterationCount) = Overlap_to_output;
    ...
    if IterationCount > convValue+1
        ...
        if round(resultoverlap1(IterationCount), convLim) <= LastOverl
            close(StopFig)
        end
    end
end
